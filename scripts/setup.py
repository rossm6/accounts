...
def run():
    ...
    bank_account = Nominal.objects.create(name="Bank Account", parent=current_assets, type="b")
    ...
    fy = FinancialYear.objects.create(financial_year=2021, number_of_periods=12)
    periods = []
    for i in range(1, 13):
        p = str(i).rjust(2, "0")
        p = Period(period=p, fy_and_period="2021" + p, month_start=date(2021,i, 1), fy=fy)
        periods.append(p)
