...
class VoidTransactionsTest(TestCase):
    ...
    def test_voiding_an_invoice_already_voided(self):
        ...
        response = self.client.post(
            reverse("sales:void", kwargs={"pk": header.pk}), data)
    ...
    # INCORRECT USAGE
    def test_voiding_a_receipt_already_voided(self):
        self.client.force_login(self.user)
    ...
    def test_voiding_a_receipt_without_matching(self):
        self.client.force_login(self.user)
    ...
    # CORRECT USAGE
    def test_voiding_a_receipt_with_matching_where_receipt_is_matched_by(self):
        self.client.force_login(self.user)

        receipt = create_receipt_with_nom_entries(
            {
                "type": "sp",
                "cash_book": self.cash_book,
                "customer": self.customer,
        ...
        invoice = create_invoices(self.customer, "inv", 1, self.period, 600)[0]
    ...
    # CORRECT USAGE
    def test_voiding_a_receipt_with_matching_where_receipt_is_matched_to(self):
        self.client.force_login(self.user)
        receipt = create_receipt_with_nom_entries(
            {
                "type": "sp",
    ...
    # CORRECT USAGE
    def test_voiding_a_brought_forward_receipt_without_matching(self):
        self.client.force_login(self.user)
    ...
    # CORRECT USAGE
    def test_voiding_a_brought_forward_receipt_with_matching_where_receipt_is_matched_by(self):
        self.client.force_login(self.user)
        ...
        data["void-id"] = receipt.pk
        response = self.client.post(
            reverse("sales:void", kwargs={"pk": receipt.pk}), data)
        self.assertEqual(response.status_code, 200)
        content = response.content.decode("utf")
        json_content = loads(content)
        self.assertEqual(
    ...
    # CORRECT USAGE
    def test_voiding_a_brought_forward_receipt_with_matching_where_receipt_is_matched_to(self):
        self.client.force_login(self.user)
    ...
    # INVALID
    def test_voiding_a_positive_receipt_so_outstanding_would_be_invalid(self):
        self.client.force_login(self.user)
        receipt = create_receipts(self.customer, "customer", 1, self.period, -1.00)[0]
        receipt1 = create_receipts(self.customer, "customer", 1, self.period, 1201)[0]
        receipt2 = create_receipts(self.customer, "customer", 1, self.period, -1200)[0]
        ...
        data = {}
        data["void-id"] = receipt1.pk
        response = self.client.post(reverse("sales:void", kwargs={"pk": receipt1.pk}), data)
        self.assertEqual(response.status_code, 200)
        content = response.content.decode("utf")
        json_content = loads(content)
        self.assertEqual(
    ...
    # INVALID
    def test_voiding_a_negative_receipt_so_outstanding_would_be_invalid(self):
        self.client.force_login(self.user)
        receipt = create_receipts(self.customer, "customer", 1, self.period, -1.00)[0]
        receipt1 = create_receipts(self.customer, "customer", 1, self.period, 1201)[0]
        receipt2 = create_receipts(self.customer, "customer", 1, self.period, -1200)[0]
        ...
        data = {}
        data["void-id"] = receipt2.pk
        response = self.client.post(reverse("sales:void", kwargs={"pk": receipt2.pk}), data)
        self.assertEqual(response.status_code, 200)
        content = response.content.decode("utf")
        json_content = loads(content)
        self.assertEqual(
    ...
    # INVALID
    def test_voiding_a_zero_receipt_INVALID(self):
        self.client.force_login(self.user)
        receipt = create_receipts(self.customer, "customer", 1, self.period, 0)[0]
        receipt1 = create_receipts(self.customer, "customer", 1, self.period, 1201)[0]
        receipt2 = create_receipts(self.customer, "customer", 1, self.period, -1200)[0]
        match(receipt, [(receipt1, -1200), (receipt2, 1200)])
